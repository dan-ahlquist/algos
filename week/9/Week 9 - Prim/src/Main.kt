...
### Header deleted by me ### [number_of_nodes] [number_of_edges]
...
simpler approach, which should already give you a healthy speed-up, is to maintain relevant edges in a heap
...
const val fileName = "edges.txt"

fun main() {
    val g = readInput(fileName)
    val mst = SpanningTree.findMst(g)
    mst.print()
}
...
fun readInput(filename: String): WeightedUndirectedGraph {
    ...
    File(filename).forEachLine {
        ...
        g.addEdge(a.toInt(), b.toInt(), weight.toInt())
    }
    return g
}